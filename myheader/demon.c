...
#define GLOBAL_M 2600
#define GLOBAL_N 2600
#define GLOBAL_K 2600
...
/* processor grid is R or C major */
...
int def_rows = PROC_NODE;
int def_columns = PROC_NODE;
...
double diff_norm(double*, double*, int, int);
...
int main (int argc, char **argv)
{
   ...
   if(ROWMAJOR)
   {
      def_columns = (int)(sqrt(nprocs));
      def_rows = nprocs/def_columns;
      while(def_columns*def_rows != nprocs)
      {
         def_columns--;
         def_rows = nprocs/def_columns; 
      }  
   ...
   {
      def_rows = (int)(sqrt(nprocs));
      def_columns = nprocs/def_rows;
      while(def_columns*def_rows != nprocs)
      {
         def_rows--;
         def_columns = nprocs/def_rows; 

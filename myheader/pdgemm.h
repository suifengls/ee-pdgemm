...
#define DESCRIPTOR_SIZE 9
#include "mpi.h"
//#include "/home/lchen/map.h"
/*===================================================================
 *
 * BLACS functions 
 */
//extern void mapping(int a, int b);
...
void Build_input_datatype(MPI_Datatype *input_datatype, int* n, int* nproc_rows, int* nproc_cols,
             int* row_block_size, int* col_block_size);

void Allocate(int my_rank, char* name, void* list, int size, 
              int datatype);

void Initialize(int p, int my_rank, double* A_local, 
...
/* Compute y = A*x */
void Mat_vect_mult(int m, int n, int k, double* A_local, int* A_descrip, 

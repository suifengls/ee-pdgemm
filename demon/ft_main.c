...
#define GLOBAL_M (22400*8/7)
#define GLOBAL_N (22400*8/7)
#define GLOBAL_K (22400*8/7)
...
int main (int argc, char **argv)
{
  //   double *A_local;
   ...
   mapping(myproc%PROC_NODE, DVFS_HIGH);
   ...
   */
   ...
   r = r - r/8;
   RndMatrix(a, local_m, local_k, r);
   RndMatrix(b, local_k, local_n, r); 
 
   if(myproc%8 == 7 || myproc >= 56)
   {
     Zeros(a, local_m, local_k);   
     Zeros(b, local_k, local_n); 
   }
   /*
   Zeros(a, local_m, local_k); 
   Zeros(b, local_k, local_n); 
   ...
   */
   ...
   mapping(myproc%PROC_NODE, DVFS_LOW);

   if(myproc == SHOW1)
   {
       sleep(1);
       //printf("Total execution time of my_pdgemm is %.3f.\n", exetime);
       printf("Total execution time of pdgemm is %.4f.\n", exetime);
   ...
   if(myproc%8 == 7)
     printf("From the last column C[100] = =%lf\n",c[100]);
   if(myproc>=56)
     printf("From the last row C[100] = =%lf\n",c[100]);
